...
# ===========================================================================
# 即將出關 - 內容寬度 + 統一 padding
# ---------------------------------------------------------------------------
# 設計原則：
#   1. 先決定每欄內容典型寬度 (content_w)
#   2. 欄寬 = content_w + 2 * SIDE_PAD（左右各加固定 padding）
#   3. 內容置中於該欄 -> 兩相鄰欄之間的視覺空白 = 2 * SIDE_PAD（恆定）
#   4. 靠右欄的數字仍然以「欄中心 + content_w/2」當作右緣
#      → 數字右緣對齊整齊，但跟下一欄的間距仍然是 2 * SIDE_PAD
#
# 為什麼這樣不會撞：
#   每欄寬度都是「內容剛好放得進 + 兩側固定 padding」
#   不會出現「pitch 不夠塞內容」的情況
#
# 為什麼間距會均勻：
#   每欄左右 padding 相等，內容置中
...
def draw_releasing_image(data, signal_map=None):
    ...
    # ---------- 前 3 欄：# / 代號 / 股票名稱 ----------
    left_col_ratios = [0.13, 0.22, 0.65]
    ...
    # ---------- 後 6 欄：依內容寬度切版 ----------
    right_block_start_x = table_left + left_block_w
    right_x_starts = []
    right_x_widths = []
    acc = right_block_start_x
    for content_w in right_col_content:
        col_w = content_w + 2 * SIDE_PAD
        right_x_starts.append(acc)
        right_x_widths.append(col_w)
        acc += col_w
    ...
    for col_idx, (label, align) in enumerate(zip(right_col_labels, right_col_aligns)):
        if align == 'right':
            ax.text(col_right_inner_x(col_idx), y_header_center, clean_display_text(label),
                    ha='right', va='center',
                    fontsize=15, fontproperties=FONT_BOLD, color=TEXT_HEADER, zorder=3)
        else:
            ax.text(col_center_x(col_idx), y_header_center, clean_display_text(label),
                    ha='center', va='center',
                    fontsize=15, fontproperties=FONT_BOLD, color=TEXT_HEADER, zorder=3)
    ...
    for i, row in enumerate(data):
        ...
        ax.add_patch(patches.Rectangle((table_left, y_top - row_h), table_w, row_h, linewidth=0, facecolor=bg_color, zorder=1))
        ax.add_patch(patches.Rectangle((left_x_starts[0], y_top - row_h), left_x_widths[0], row_h, linewidth=0, facecolor=BG_RANK, zorder=1))
        ax.plot([table_left + 0.1, table_right - 0.1], [y_top - row_h, y_top - row_h], color=BORDER_DARK, linewidth=0.6, zorder=2)
        ...
        # [0] 現價 - 靠右（右緣對齊到 col_right_inner_x）
        ...
        # [1] 倒數交易日 - 置中（膠囊）
        bg_clr, fg_clr = get_days_style(days)
        capsule_w, capsule_h = DAYS_W, 0.28
        capsule_x = col_center_x(1) - capsule_w / 2
        capsule_y = y_center - capsule_h / 2
        ...
        group_left = col_center_x(2) - total_w / 2
        ...
        if emoji_ok:
            ...
            ax.text(col_center_x(2), y_center, f"{icon_fallback} {status_text}",
                    ha='center', va='center',
                    fontsize=15, fontproperties=FONT_BOLD, color=st_color, zorder=3)

        # [3] 處置前 - 靠右
        ax.text(col_right_inner_x(3), y_center, f"{pre_pct}%",
                ha='right', va='center',
                fontsize=15, fontproperties=FONT_BOLD, color=get_pct_color(pre_pct), zorder=3)

        # [4] 處置中 - 靠右
        ax.text(col_right_inner_x(4), y_center, f"{in_pct}%",
                ha='right', va='center',
                fontsize=15, fontproperties=FONT_BOLD, color=get_pct_color(in_pct), zorder=3)

        # [5] 出關日 - 置中
        ax.text(col_center_x(5), y_center, date,
                ha='center', va='center',
