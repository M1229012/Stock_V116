# 📈 台股注意股與處置股預測系統 (V116.30)

> **⚖️ 法規同步（115.08.10 新制）**
> 依證交所 115.08.03 公告修正之「公布或通知注意交易資訊暨處置作業要點」第 6 條，
> 暨「第四條異常標準之詳細數據及除外情形」第 12 條，自 **115 年 8 月 10 日**施行：
>
> | 項目 | 舊制 | 新制 |
> | :--- | :--- | :--- |
> | 一般處置期間（第一次／第二次含以上） | 10 個營業日 | **5 個營業日** |
> | 併同「當沖占比過高」之處置期間 | 12 個營業日 | **7 個營業日** |
> | 處置期間撮合頻率 | 每 5 分鐘／每 20 分鐘 | **約每 2 分鐘** |
> | 第 11 款 6 日收盤價起迄價差 | 收盤價 ≥ 500 元適用，門檻 100 元，每 500 元級距 +25 元 | **收盤價逾 1,000 元才適用，門檻 300 元；逾 2,000 元後每 1,000 元級距 +150 元** |
>
> **注意次數達到處置的門檻本次未修正**，仍為「連續 3 日第 1 款／連續 5 日／10 日內 6 次／30 日內 12 次」（要點第 6 條明定僅累計第四條第一項**第 1～8 款**）。
>
> 連帶調整：因處置期間縮短為 5 個營業日，「即將出關監控」預警窗口由 5 個交易日改為 **3 個交易日**（`RELEASE_ALERT_TRADING_DAYS`），否則個股一進處置即落入名單、篩選失去意義。
...
系統會回朔過去 30 個交易日，依據證交所規定之「連續 3 日」、「連續 5 日」、「10 日內 6 次」、「30 日內 12 次」等條件進行模擬（**此組門檻於 115.08.10 新制未修正**）。
* **0 天**：代表今日已達標（次一營業日進處置）或目前正在處置中。
* **1 天**：代表再觸發 1 次即進入處置。

達標後的處置期間為 **5 個營業日**；若計算期間內同時因「當沖交易占比過高」（第 13 款）被公布注意，處置期間為 **7 個營業日**，系統會在「處置觸發原因」欄位加註提示。
天數常數集中於 `main.py` 的 `DISPOSAL_DAYS_NORMAL` / `DISPOSAL_DAYS_DAYTRADE`，未來法規再調整時只需改這兩個值。

> 註：實際「處置起訖日」一律以官方「處置股 90 日明細」公告為準，系統不以天數推算，因此出關監控與技術追蹤不受天數調整影響。
