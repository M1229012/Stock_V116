        ...
        if r.status_code == 200:
            ...
            if rows:
                ...
                for row in rows:
                    ...
                    # 3. 期間 (往後找含區間分隔符：~ / ～ / – / -，且需含日期格式)
                    for k in range(found_code_idx + 2, len(row)):
                        s_item = str(row[k]).strip()
                        if ("~" in s_item) or ("～" in s_item) or ("–" in s_item) or ("-" in s_item):
                            if re.search(r"\d{2,3}/\d{1,2}/\d{1,2}", s_item):
                                c_period = s_item
                                break
...
async def run_jail_crawler_pipeline():
    ...
    if all_dfs:
        ...
        def parse_sort_date(period_str):
            try:
                # 取區間的起始日
                start_part = period_str.replace("～", "-").replace("~", "-").replace("–", "-").split("-")[0].strip()
                if "/" in start_part:
